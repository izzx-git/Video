...
;патчи драйвера
	org #71ec ;здесь чтение сектора ZC
	jp read_sec_zc
	...
	org #78ed ;здесь чтение сектора smuc без #3d2f
	jp read_sec_smuc
	; ld a,#20
	; ;ex af,af'
	; dup 8
...
;ЖЕЛАТЕЛЬНО ЗАПРЕЩАТЬ НА ВРЕМА РАБОТЫ
	...
	ld hl,snd_adr ;чтобы не было щелчков звук играть отсюда
	ld (snd_adr_cur),hl
...
;И ПОСЛЕ СКАНИРОВАНИЯ
	...
	;Включить Covox в GS
	ld a,#0e ;#0E Go to LPT Covox mode
	;call SENDCOM
	out (GSCOM),a
...
; dev
	...
	di
	ld hl,loadpic ;адрес обработчика
	ld ((interrupt_vec/256)*256+#ff),hl 
...
loadpic ;цикл проигрывателя
;1й
	pop af ;сюда возврат при прерывании
	
loadpic_
		ld (#4000),hl ;для отладки
	exx	
...
select_scr7
		...
		ld hl,snd_adr ;адрес звука		
		ld a,(cur_scr) ;смена звука		
		or a
		jr z,select_snd
		ld (snd_adr_cur),hl
		ld a,h
		add 8 ;2048 вместо 4096
		ld h,a
		jr select_snd2
select_snd
		ld a,h
		...
		ld hl,snd_adr ;адрес звука	
select_snd2		
		ld a,4 ;секторов, главное уместить до адреса #af00 
		;LD HL,(snd_adr_сur) ;адрес звука	
	...
	exx			
		...
		ld a,5 ;дополнительно тянем время, пустая загрузка
		LD HL,#c000	
...
; add_sound_skip		
		...
		CPL
		AND #1F
		JR nz,pause ;пауза или стоп

cont
		;теперь выводить только звук до прерывания
		ei
	exx
	outi ;байт звука
	ld (snd_adr_cur),hl
...
pause ;пауза или останов
		ld a,#7f
		IN A,(#FE)
		CPL
		AND 1
		JR z,exit_ ;выход если не пробел
release		
		ld b,32
release1 ;если пробел
		dec bc
		ld a,c
		or b
		jr nz,release1
		...
		CPL
		AND #1F
		JR nz,release 	;подождать пока клавишу отпустят
pause2		
		ld b,32		
pause1	
		dec bc
		...
		CPL
		AND #1F
		JR nz,cont ;любая клавиша продолжить
		jr pause2
...
calc_cur_pos_zx
		...
		and a
		sbc hl,bc
		jp c,exit_zx ;выход если файл кончился	
		jr nz,chek_skip_zx
		ld hl,(len_file_l) ;размер файла младшие байты
		ld bc,(cur_file_pos_l) ;текущий
		and a
...
; add_sound_skip_zx
		...
		CPL
		AND #1F
		jp nz,exit_zx ;выход по любой клавише
...
read_sec_zc ;чтение сектора с одновременным выводом звука через zc
	exx
	ld bc,(snd_port) ;порт для звука
	ld hl,(snd_adr_cur) ;адрес 
	exx
...
select_dev ;выбор устройства и раздела
	...
	add a,"1"-1 ;подготовить сообщение выбора разделов
...
right; на страницу вниз
			...
			jr c,right01_1 ;если больше 255
            cp     c
...
catposit_r ;позиция печати каталога справа
	db 22,0,30,0
mes_title
	db 22,0,13,"VPlayer 0.1.6.1"
	db 22,2,13,"Arrow keys"
	db 22,3,13,"R - rescan"
	db 22,4,13,"En - play"
	db 22,5,13,"Sp - pause"
	db 22,6,13,"Any - stop"
	db 22,8,13,"S - sound port:"
mes_b3	db 22,9,13,256-5," B3 (GS)"
mes_dd	db 22,10,13,"  DD (LPT)"
	db 0
...
file_name_ext_GMM db "..." ;"GMM" ;расширение отключено
file_name_ext_GMV db "GMV" ;расширение
file_name_ext_ZXV db "..." ;"ZXV" ;расширение отключено
...
snd_adr_cur dw 0 ;текущий адрес звука
snd_port dw #b3 ;порт для звука
...
interrupt_vec equ $-1 ;указатель на обработчик прерываний
		ds 1 ;отступ от вектора прерываний
		align 256
snd_adr ;адрес звука	
		ds 2048+1024
snd_adr2 ;адрес звука нечётный кадр
